"""
plot_flat_trot.py  –  FIXED VERSION

Post-process and visualize controller response for
Phase 2–6 Convex MPC Trot (Go2) on flat ground.
...
SAVE_DIR  = "results/flat"      # where to write PNGs
...
CONTACT_THRESHOLD = 10.0   # N — GRF threshold to determine stance vs swing

def plot_contacts(df, ax):
    leg_names = ["FL", "FR", "RL", "RR"]
    colors    = ACCENT[:4]
    for i, (leg, col) in enumerate(zip(leg_names, colors)):
        # Derive contact from GRF: foot is in stance when Fz > threshold.
        # The logged contact_* columns are all-1 (logging bug); GRF is ground truth.
